...
impl XAtom {
    ...
    pub fn get_name(&self, atom: xlib::Atom) -> &str {
        match atom {
            _ if atom == self.WMProtocols => "WM_PROTOCOLS",
            _ if atom == self.WMDelete => "WM_DELETE_WINDOW",
            _ if atom == self.WMState => "WM_STATE",
            _ if atom == self.WMTakeFocus => "WM_TAKE_FOCUS",
            _ if atom == self.NetActiveWindow => "_NET_ACTIVE_WINDOW",
            _ if atom == self.NetSupported => "_NET_SUPPORTED",
            _ if atom == self.NetWMName => "_NET_WM_NAME",
            _ if atom == self.NetWMState => "_NET_WM_STATE",
            _ if atom == self.NetWMStateModal => "NetWMStateModal",
            _ if atom == self.NetWMStateSticky => "NetWMStateSticky",
            _ if atom == self.NetWMStateMaximizedVert => "NetWMStateMaximizedVert",
            _ if atom == self.NetWMStateMaximizedHorz => "NetWMStateMaximizedHorz",
            _ if atom == self.NetWMStateShaded => "NetWMStateShaded",
            _ if atom == self.NetWMStateSkipTaskbar => "NetWMStateSkipTaskbar",
            _ if atom == self.NetWMStateSkipPager => "NetWMStateSkipPager",
            _ if atom == self.NetWMStateHidden => "NetWMStateHidden",
            _ if atom == self.NetWMStateFullscreen => "NetWMStateFullscreen",
            _ if atom == self.NetWMStateAbove => "NetWMStateAbove",
            _ if atom == self.NetWMStateBelow => "NetWMStateBelow",
            _ if atom == self.NetWMWindowType => "_NET_WM_WINDOW_TYPE",
            _ if atom == self.NetWMWindowTypeDialog => "_NET_WM_WINDOW_TYPE_DIALOG",
            _ if atom == self.NetWMWindowTypeDock => "_NET_WM_WINDOW_TYPE_DOCK",
            _ if atom == self.NetClientList => "_NET_CLIENT_LIST",
            _ if atom == self.NetDesktopViewport => "_NET_DESKTOP_VIEWPORT",
            _ if atom == self.NetNumberOfDesktops => "_NET_NUMBER_OF_DESKTOPS",
            _ if atom == self.NetCurrentDesktop => "_NET_CURRENT_DESKTOP",
            _ if atom == self.NetDesktopNames => "_NET_DESKTOP_NAMES",
            _ if atom == self.NetWMDesktop => "_NET_WM_DESKTOP",
            _ if atom == self.NetWMStrutPartial => "_NET_WM_STRUT_PARTIAL",
            _ if atom == self.NetWMStrut => "_NET_WM_STRUT",
            _ => "(UNKNOWN)",
        }
    }
